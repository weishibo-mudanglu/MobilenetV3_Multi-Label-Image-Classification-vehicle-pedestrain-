...
class SqueezeAndExciteAndCBAM(nn.Module):
    def __init__(self, in_channels, out_channels,se_kernel_size, divide=4):
        super(SqueezeAndExciteAndCBAM, self).__init__()
        mid_channels = in_channels // divide
        self.avgpool = nn.AvgPool2d(kernel_size=se_kernel_size,stride=1)
        self.maxpool = nn.MaxPool2d(kernel_size=se_kernel_size,stride=1)
        self.CBAMavgpool = nn.AvgPool2d(kernel_size=(out_channels,1),stride=1)
        self.CBAMmaxpool = nn.MaxPool2d(kernel_size=(out_channels,1),stride=1)
    ...
    def forward(self, x):
        ...
        out_avg = out_avg.view(b, -1)
        out_max = out_max.view(b, -1)
        out_avg = self.SEblock(out_avg)
        out_max = self.SEblock(out_max)
        out = out_avg + out_max
        out = out.view(b, c, 1, 1)
        out = out * x
        out = torch.transpose(out,1,2)
        cha_avg = self.CBAMavgpool(out)
        cha_max = self.CBAMmaxpool(out)
        out = torch.cat((cha_avg,cha_max),dim = 2)
        out = torch.transpose(out,1,2)
        out = self.CBAM(out)
        return out * x
...
class SEInvertedBottleneck(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels, kernel_size, stride,activate, use_se, se_kernel_size=1):
        ...
        if self.use_se:
            self.SEblock = SqueezeAndExciteAndCBAM(mid_channels, mid_channels, se_kernel_size)
...
class MobileNetV3(nn.Module):
    def __init__(self, num_classes=1000,type='large'):
        ...
        self.lstm3 = nn.LSTM(input_size=10, hidden_size=10, num_layers=1,batch_first=True)
        self.lstm4 = nn.LSTM(input_size=10, hidden_size=10, num_layers=1,batch_first=True)
        self.lstm5 = nn.LSTM(input_size=10, hidden_size=10, num_layers=1,batch_first=True)
        self.lstm6 = nn.LSTM(input_size=10, hidden_size=10, num_layers=1,batch_first=True)
        self.lstm7 = nn.LSTM(input_size=10, hidden_size=10, num_layers=1,batch_first=True)
        self.lstm8 = nn.LSTM(input_size=10, hidden_size=10, num_layers=1,batch_first=True)
        self.lstm9 = nn.LSTM(input_size=10, hidden_size=10, num_layers=1,batch_first=True)
        self.lstm10= nn.LSTM(input_size=10, hidden_size=10, num_layers=1,batch_first=True)
    ...
    def forward(self, x):
        ...
        #串联不同层的LSTM
        # out_fmale,(h_1,c_1) = self.lstm1(out_fmale.unsqueeze(1))
        # out_downclothing,(h_2,c_2) = self.lstm5(out_downclothing.unsqueeze(1),(h_1,c_1))
        # out_handBag,(h_3,c_3) = self.lstm8(out_handBag.unsqueeze(1),(h_2,c_2))
        # out_shoulderbag,(h_4,c_4) = self.lstm9(out_shoulderbag.unsqueeze(1),(h_3,c_3))
        # out_dirction,(h_5,c_5) = self.lstm3(out_dirction.unsqueeze(1),(h_4,c_4))
        # out_glass,(h_6,c_6) = self.lstm7(out_glass.unsqueeze(1),(h_5,c_5))
        # out_carry,(h_7,c_7)= self.lstm10(out_carry.unsqueeze(1),(h_6,c_6))
        # out_hat,(h_8,c_8) = self.lstm6(out_hat.unsqueeze(1),(h_7,c_7))
        # out_upclothing,(h_9,c_9) = self.lstm4(out_upclothing.unsqueeze(1),(h_8,c_8))
        # out_age,(h_10,c_10) = self.lstm2(out_age.unsqueeze(1),(h_9,c_9))
        #每个分支分类数目可能不同
        pre_out_fmale = self.reclassify_one1(out_fmale.squeeze(1))
        pre_out_age = self.reclassify_three7(out_age.squeeze(1))
        pre_out_dirction = self.reclassify_three8(out_dirction.squeeze(1))
        pre_out_upclothing = self.reclassify_two9(out_upclothing.squeeze(1))
        pre_out_downclothing = self.reclassify_three10(out_downclothing.squeeze(1))
        pre_out_hat = self.reclassify_one2(out_hat.squeeze(1))
        pre_out_glass = self.reclassify_one3(out_glass.squeeze(1))
        pre_out_handBag = self.reclassify_one4(out_handBag.squeeze(1))
        pre_out_shoulderbag = self.reclassify_one5(out_shoulderbag.squeeze(1))
        pre_out_carry = self.reclassify_one6(out_carry.squeeze(1))
        ...
        return out_fmale.squeeze(1),out_hat.squeeze(1),out_glass.squeeze(1),out_handBag.squeeze(1),out_shoulderbag.squeeze(1),out_carry.squeeze(1),out_age.squeeze(1),out_dirction.squeeze(1),out_upclothing.squeeze(1),out_downclothing.squeeze(1) \
