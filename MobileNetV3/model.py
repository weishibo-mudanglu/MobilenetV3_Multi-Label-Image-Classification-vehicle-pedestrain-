...
class Bottleneck(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size,exp_channels,stride,se='True',nl='HS'):
        ...
        if stride == 1 and in_channels is not out_channels:
            self.shortcut = nn.Sequential(
                # 下面的操作卷积不改变尺寸和通道数
...
class MobileNetV3_large(nn.Module):
    ...
    def __init__(self,num_classes=17):
        ...
        self.linear2 = nn.Linear(1280,5)
        self.linear3 = nn.Linear(1280,10)
...
class MobileNetV3_small(nn.Module):
    ...
    def __init__(self,num_classes=17):
        ...
        self.se = SEModule(576)
        # 卷积后不跟BN，就应该把bias设置为True
        self.conv3=nn.Conv2d(576,1280,1,1,padding=0,bias=True)
        self.conv4_1=nn.Conv2d(1280,10,1,stride=1,padding=0,bias=True)
        self.conv4_2=nn.Conv2d(1280,10,1,stride=1,padding=0,bias=True)
        self.conv4_3=nn.Conv2d(1280,10,1,stride=1,padding=0,bias=True)
    ...
    def forward(self,x):
        ...
        out=Hswish(self.se(out)) 
        # out = F.avg_pool2d(out, 7)
        out=F.avg_pool2d(out,2)
        out = Hswish(self.conv3(out))
        dirction = self.conv4_1(out)
        type = self.conv4_2(out)
        color = self.conv4_3(out)
        out1 = self.linear1(dirction.squeeze(-1).squeeze(-1))
        out2 = self.linear2(type.squeeze(-1).squeeze(-1))
        out3 = self.linear3(color.squeeze(-1).squeeze(-1))
        # 因为原论文中最后一层是卷积层来实现全连接的效果，维度是四维的，后两维是1，在计算损失函数的时候要求二维，因此在这里需要做一个resize
        # a, b = out.size(0), out.size(1)
        # out = out.view(a, b)
        return dirction,type,color,out1,out2,out3
